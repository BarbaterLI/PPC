...
class MemoryCache:
    ...
    def set(self, key: str, value: Any, ttl: float | None = None) -> bool:
        ...
        expires_at = None if ttl is None or ttl == 0 else current_time + ttl
        ...
        with self._lock:
            if key in self._cache:
                ...
                if net_increase > 0:
                    while self._current_size_bytes + net_increase > self._max_size_bytes and len(self._cache) > 1:
                        self._evict_lru()
                ...
                if size_bytes > 0:
                    while self._current_size_bytes + size_bytes > self._max_size_bytes and len(self._cache) > 0:
                        self._evict_lru()
...
class DiskCache:
    ...
    def _maybe_save_metadata(self) -> None:
        ...
        should_save = self._metadata_dirty and (
            current_time - self._last_metadata_save >= self._metadata_save_interval
            or self._metadata_op_count >= self._save_metadata_threshold
        )
    ...
    def set(self, key: str, value: bytes, ttl: float | None = None) -> bool:
        """Set cached value
        ...
        with self._lock:
            ...
            while self._current_size_bytes + size_bytes > self._max_size_bytes and len(self._metadata) > 0:
    ...
    def _evict_if_needed(self) -> int:
        ...
        oldest_key = min(self._metadata.keys(), key=lambda k: self._metadata[k].last_access_time)
...
class MultiLevelCache:
    ...
    def set(
        ...
        with self._lock:
            ...
            if CacheLevel.L1_MEMORY in levels and not self.l1.set(key, value, ttl):
                success = False
        ...
        while self._running:
            try:
                ...
                if l1_cleaned > 0 or l2_cleaned > 0:
                    logger.debug(f"Cache cleanup completed: L1={l1_cleaned}, L2={l2_cleaned}")
            except Exception as e:
