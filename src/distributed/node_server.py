...
架构设计:
- 轻量级 HTTP 服务 (aiohttp)
- TTS 工作交给 :class:`src.distributed.processing_unit.WorkerUnit`
  （它使用与 ``ppc10 convert`` 相同的 ``TTSExecutor``，因此复用了
  断点续传、并发预热、限流、隔离等能力）。
- 兼容旧 ``/api/v1/synthesize``（按文本一次性合成），由
  :class:`src.engines.tts_engine.TTSEngine` 直接处理。
    ...
    async def handle_convert(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            try:
                ...
                return web.json_response({"error": f"missing required field: {missing}"}, status=400)
            except Exception as e:  # noqa: BLE001
                return web.json_response({"error": f"invalid convert request: {e}"}, status=400)
            ...
            self._stats.record_request(
                ...
                duration=duration,
                bytes_processed=sum((p.stat().st_size if p.exists() else 0) for p in result.output_files),
            )
            ...
            return web.json_response({"error": str(e), "node_id": self.node_id}, status=500)
