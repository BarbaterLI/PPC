...
from src.config.presets import get_preset, get_preset_names
...
class ConfigManager:
    ...
    def __init__(self, config_dir: Path | None = None):
        ...
        self._config: PPC10Config | None = None
        ...
        with self._lock:
            ...
            if len(keys) >= 2 and section_name in section_models:
                ...
                try:
                    ...
                    raise ValueError(f"配置值验证失败: {key} - {'; '.join(errors)}") from None
    ...
    def set_temp(self, key: str, value: Any):
        with self._lock:
            ...
            from src.config.manager_io import load_config_from_file
    ...
    def update_config(self, key: str, value: Any, source: str = "manual") -> bool:
        with self._lock:
            try:
                ...
                event = ConfigChangeEvent(
                    key=key, old_value=old_value, new_value=value, timestamp=datetime.now(timezone.utc), source=source
                )
    ...
    def _load(self):
        ...
        with self._lock:
            ...
            if is_new_config:
                from src.config.manager_io import save_commented_config
