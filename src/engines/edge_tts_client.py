...
* an error taxonomy mapping ``edge_tts`` / network failures to the
  :mod:`src.core.exceptions` hierarchy (``TransientError`` /
...
try:
    ...
    EdgeTTSException = Exception  # type: ignore[misc,assignment]
    NoAudioReceived = Exception  # type: ignore[misc,assignment]
    SkewAdjustmentError = Exception  # type: ignore[misc,assignment]
    UnexpectedResponse = Exception  # type: ignore[misc,assignment]
    UnknownResponse = Exception  # type: ignore[misc,assignment]
    WebSocketError = Exception  # type: ignore[misc,assignment]
...
class EdgeTTSClient(ABC):
    ...
    async def synthesize_stream(
        ...
        *,
        ...
        :mod:`src.core.exceptions` taxonomy.
        """
        if last_chunk_offset and last_chunk_offset < 0:
            raise PermanentError(f"last_chunk_offset must be non-negative, got {last_chunk_offset}")

        offset = int(last_chunk_offset or 0)
        try:
            async for chunk in self._synthesize(text, voice, rate=rate, volume=volume):
...
class EdgeTTSHttpClient(EdgeTTSClient):
    ...
    def __init__(
        ...
        *,
        ...
        list_voices_factory: Any | None = None,
    ) -> None:
        super().__init__(
    ...
    def _make_communicate(self, text: str, voice: str, rate: str, volume: str) -> Any:
        ...
        if not _EDGE_TTS_AVAILABLE:
            raise PermanentError("edge-tts is not installed; cannot construct Communicate")
        ...
        *,
        ...
        for entry in entries or []:
            ...
            if gender and g.lower() != gender.lower():
                continue
            name = str(entry.get("Name", short) if isinstance(entry, dict) else getattr(entry, "Name", short))
            locale_val = str(
                entry.get("Locale", short.split("-")[0] if short else "")
                if isinstance(entry, dict)
                else getattr(entry, "Locale", short.split("-")[0] if short else "")
            )
            friendly = str(
                entry.get("FriendlyName", short) if isinstance(entry, dict) else getattr(entry, "FriendlyName", short)
            )
            voices.append(
                VoiceInfo(
                    name=name,
