...
class AnalysisEngine:
    ...
    def get_analyzer(self, name: str) -> BaseAnalyzer | None:
    ...
    def _select_analyzers(self, names: list[str] | None = None) -> list[BaseAnalyzer]:
        """Select analyzers to run."""
        if names is not None:
            return [self._analyzers[n] for n in names if n in self._analyzers and self._analyzers[n].enabled]
    ...
    def get_issues_by_category(self, report: HealthReport | None = None) -> dict[AnalysisCategory, list[AnalysisIssue]]:
    ...
    def get_top_issues(self, limit: int = 10, report: HealthReport | None = None) -> list[AnalysisIssue]:
