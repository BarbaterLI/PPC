...
class AnalysisHistoryManager:
    ...
    def __init__(self, storage_dir: Path | None = None) -> None:
        self._storage_dir = Path.home() / ".ppc10" / "analysis_history" if storage_dir is None else storage_dir
    ...
    def compare(self, report_id: str) -> dict[str, Any] | None:
        ...
        * ``persistent_issues`` —issues that appear in both reports.
        ...
        return {
            ...
            "new_issues": [
                i.to_dict() for i in current.issues if i.description in current_descriptions - historical_descriptions
            ],
            "fixed_issues": [
                i.to_dict()
                for i in historical.issues
                if i.description in historical_descriptions - current_descriptions
            ],
            "persistent_issues": [
                i.to_dict() for i in current.issues if i.description in (current_descriptions & historical_descriptions)
            ],
    ...
    def _load_index(self) -> dict[str, Any]:
        ...
        if not self._index_file.is_file():
            return {}
        try:
            return json.loads(self._index_file.read_text(encoding="utf-8"))  # type: ignore[no-any-return]  # json.loads 返回 Any
