...
class ConfigAnalyzer(BaseAnalyzer):
    ...
    async def analyze(self, context: dict[str, Any] | None = None) -> list[AnalysisIssue]:
        ...
        if concurrency is not None and timeout is not None and concurrency > 16 and timeout < 60:
            issues.append(
                AnalysisIssue(
        ...
        if retries is not None and auto_retry is not None and retries == 0 and auto_retry:
            issues.append(
                AnalysisIssue(
        ...
        if rate_limit is not None and buffer_size is not None and rate_limit > 200 and buffer_size < 16:
            issues.append(
                AnalysisIssue(
                    severity=Severity.MEDIUM,
                    category=AnalysisCategory.CONFIGURATION,
        ...
        if timeout_min is not None and timeout_max is not None and timeout_min > timeout_max:
            issues.append(
                AnalysisIssue(
                    severity=Severity.CRITICAL,
                    category=AnalysisCategory.CONFIGURATION,
        ...
        if tts_retries is not None and reliability_retries is not None and tts_retries != reliability_retries:
            issues.append(
                AnalysisIssue(
                    severity=Severity.MEDIUM,
                    category=AnalysisCategory.CONFIGURATION,
