...
def _is_rare(ch: str) -> bool:
    ...
    if cat in {"Pd", "Ps", "Pe", "Pc", "Po"}:
        return False
    return cat not in {"Zs", "Sm", "Sc", "Sk", "So"}
...
class TextQualityAnalyzer(BaseAnalyzer):
        ...
        if not texts:
            ...
            return [
                ...
                AnalysisIssue(
                    severity=Severity.INFO,
                    category=AnalysisCategory.CODE_QUALITY,
                    description=("文本质量汇总: 可读性 100/100, 可合成性 100/100, 综合 100/100"),
                    suggestion="无文本可分析",
            ...
            for key in aggregated:
                if isinstance(score[key], (int, float)):
                    aggregated[key] = (
                        ...
                        if key in {"long_sentences", "long_lines", "control_chars", "char_count", "sentence_count"}
                        else aggregated[key] + score[key]
                    )
            self._emit_issues(label, content, score, issues)

        # Average ratios/scores
        for key in ("readability", "synthesizability", "overall", "rare_char_ratio", "oov_ratio"):
    ...
    def _score_text(self, text: str) -> dict[str, Any]:
        ...
        score["long_sentences"] = sum(1 for s in sentences if len(s) > self._long_sentence_threshold)
        ...
        # Long lines
        score["long_lines"] = sum(1 for line in text.splitlines() if len(line) > self._max_line_length)
        ...
        if score["oov_ratio"] > OOV_RATIO_PENALTY_THRESHOLD:
            issues.append(
                AnalysisIssue(
                    ...
                    category=AnalysisCategory.CODE_QUALITY,
                    description=(f"{label}: OOV 字符占比 {score['oov_ratio'] * 100:.2f}%"),
        ...
        if score["long_lines"] > 0:
            issues.append(
                AnalysisIssue(
                    severity=Severity.MEDIUM,
                    category=AnalysisCategory.CODE_QUALITY,
                    description=(f"{label}: {score['long_lines']} 行超过 {self._max_line_length} 字符"),
        ...
        if score["control_chars"] > 0:
            issues.append(
                AnalysisIssue(
                    severity=Severity.HIGH,
                    category=AnalysisCategory.CODE_QUALITY,
                    description=(f"{label}: 检测到 {score['control_chars']} 个控制字符"),
