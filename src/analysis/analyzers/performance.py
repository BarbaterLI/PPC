...
async def _run_pyspy_flamegraph(
    ...
    target_path = output_path or os.path.join(tempfile.gettempdir(), f"ppc10_flamegraph_{target_pid}.svg")
    ...
    try:
        ...
        _, stderr = await asyncio.wait_for(proc.communicate(), timeout=duration + 30.0)
...
async def _run_scalene_flamegraph(
    ...
    """
    start = asyncio.get_event_loop().time()
    target_path = output_path or os.path.join(tempfile.gettempdir(), "ppc10_scalene_profile.json")
    try:
        cmd = [
            "python",
            "-m",
...
async def capture_flamegraph(
    ...
    if chosen == "py-spy":
        return await _run_pyspy_flamegraph(pid=pid, duration=duration, output_path=output_path)
    if chosen == "scalene":
        return await _run_scalene_flamegraph(output_path=output_path, duration=duration)
    ...
    return _build_self_flamegraph(stats)
...
class PerformanceAnalyzer(BaseAnalyzer):
    ...
    def __init__(self) -> None:
        ...
        self._last_flamegraph: FlameGraphResult | None = None

    @property
    def last_flamegraph(self) -> FlameGraphResult | None:
            ...
            if fg.success:
                issues.append(
                    AnalysisIssue(
                        ...
                        category=AnalysisCategory.PERFORMANCE,
                        description=(f"已生成火焰图 (backend={fg.backend}, duration={fg.duration_seconds:.2f}s)"),
