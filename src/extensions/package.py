    ...
    try:
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            return ExtensionManifest(
                ...
                min_ppc10_version=raw.get("min_ppc10_version", "10.1.0"),
                tags=raw.get("tags", []),
            )
    except zipfile.BadZipFile:
        raise ValueError(f"文件不是有效的 zip 包: {zip_path}") from None
...
def validate_package(zip_path: Path) -> tuple[bool, list[str]]:
    ...
    with zf:
        ...
        min_ver = manifest_data.get("min_ppc10_version", "10.1.0")
        try:
            current = pkg_version.parse(ppc10.__version__)
            required = pkg_version.parse(min_ver)
            if current < required:
                errors.append(f"当前 PPC10 版本 {ppc10.__version__} 低于扩展要求的最低版本 {min_ver}")
...
class ExtensionPackageManager:
        ...
        try:
            ...
            with zipfile.ZipFile(zip_path, "r") as zf:
                ...
                for info in zf.infolist():
                    ...
                    if (
                        len(parts) > 1
                        and Path(info.filename).name != "manifest.yml"
                        and Path(info.filename).name != manifest.entry
                    ):
                        Path(*parts[1:])
                    else:
                        Path(info.filename)
    ...
    def create_template(self, name: str, output_dir: Path | None = None) -> Path:
        ...
        extension_content = (
            f'"""{name} 扩展"""\n'
            f"\n"
            f"from src.extensions.base import Extension, ExtensionMetadata, ExtensionType, ToolIntegration\n"
            f"\n"
            f"\n"
    ...
    def _register_in_config(self, manifest: ExtensionManifest, zip_path: Path) -> None:
        ...
        try:
            ...
            from src.config.schema import InstalledExtensionInfo
