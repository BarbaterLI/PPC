...
def _compute_config_signature(content: str) -> str:
    ...
    return hmac.new(_get_sign_key(), content.encode("utf-8"), hashlib.sha256()).hexdigest()  # type: ignore[call-overload,no-any-return]  # hashlib.sha256() 实例被 mypy stubs 误判为 HMAC 参数
...
def get_latest_release(owner: str = "zhongbai2333", repo: str = "Tomato-Novel-Downloader") -> dict:
    ...
    try:
        ...
        return response.json()  # type: ignore[no-any-return]  # requests stubs 返回 Any，运行时已知为 dict
    except requests.exceptions.RequestException as e:
        raise RuntimeError(f"获取最新 Release 失败: {e}") from None
...
def get_releases_list(
    ...
    try:
        ...
        response.raise_for_status()
        return response.json()  # type: ignore[no-any-return]  # requests stubs 返回 Any，运行时已知为 list
    except requests.exceptions.RequestException as e:
        raise RuntimeError(f"获取 Release 列表失败: {e}") from None
...
def match_best_asset(
    ...
    for asset in assets:
        name = asset["name"]
        if ("Source" in name and name.endswith(".zip")) or ("Source" in name and name.endswith(".tar.gz")):
            continue
...
def install_fanqie(
    ...
    official_url = cast(str, best_asset["browser_download_url"])
    mirror_url = convert_to_mirror_url(official_url, mirror) if use_mirror else None
    filename = cast(str, best_asset["name"])
    ...
    if use_mirror:
        primary_url: str = mirror_url or official_url
        fallback: str = official_url
    else:
...
def _extract_and_install(archive_path: Path, target_dir: Path) -> bool:
    from src.utils.files import safe_extract_zip
                ...
                for member in tf.getmembers():
                    if member.isfile():
                        ...
                        if extracted is None:
                            continue
                        with extracted, open(dest, "wb") as dst:
...
def update_fanqie(
    ...
    from src.extensions.fanqie.downloader_core import get_installed_version
...
def uninstall_fanqie() -> bool:
    from src.extensions.fanqie.downloader_core import _get_fanqie_base_dir
...
def _get_fanqie_base_dir() -> Path:
    from src.extensions.fanqie.downloader_core import _get_fanqie_base_dir as get_base
...
def _get_exe_path() -> Path:
    from src.extensions.fanqie.downloader_core import _get_exe_path as get_exe
...
def is_installed() -> bool:
    from src.extensions.fanqie.downloader_core import is_installed as check_installed
