...
class CronScheduler:
    ...
    def __init__(self, storage_path: Path | None = None):
        ...
        if not CRON_AVAILABLE:
            logger.warning("croniter 未安装，Cron 表达式解析功能将受限。\n安装命令: pip install croniter")
    ...
    def enable_task(self, task_id: str) -> bool:
        ...
        with self._lock:
            if task_id in self._tasks:
                ...
                if CRON_AVAILABLE:
                    self._tasks[task_id].next_run = self._calculate_next_run(self._tasks[task_id].cron_expr)
                self._save_tasks()
                return True
            return False
    ...
    def _calculate_next_run(self, cron_expr: str) -> datetime | None:
        """计算下次执行时间"""
        if not CRON_AVAILABLE:
            return None
    ...
    def _execute_convert(self, params: dict[str, Any]) -> bool:
        """执行转换命令"""
        try:
            from src.cli.commands.convert import handle_convert

            handle_convert(
                input=Path(params.get("input_dir", "")),
                output=Path(params.get("output_dir", "")) if params.get("output_dir") else None,
                ...
                with self._lock:
                    for task in self._tasks.values():
                        if task.enabled and task.next_run and task.next_run <= now:
                            threading.Thread(target=self._run_and_update, args=(task.id,), daemon=True).start()
