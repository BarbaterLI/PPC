...
class PriorityScheduler:
    ...
    async def wait_for(self, task_id: str, timeout: float | None = None) -> Any:
        ...
        try:
            ...
            raise TimeoutError(f"Wait for task {task_id} timed out") from None
    ...
    async def _worker(self, worker_id: int) -> None:
        ...
        while self._running:
            try:
                priority, task_id, func, args, kwargs = await asyncio.wait_for(self._queue.get(), timeout=1.0)
            except asyncio.TimeoutError:
                continue
    ...
    async def _execute_task(
        ...
        try:
            ...
            logger.debug(f"Task completed: {task_id} ({task.name}), execution time: {task.execution_time:.2f}s")
        except asyncio.TimeoutError:
            async with self._lock:
