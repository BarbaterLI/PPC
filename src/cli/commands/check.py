...
提供系统环境、依赖包、网络连通性、文件系统、系统资源和配置的全面检查，
支持报告导出和一键修复功能。
...
class SystemChecker:
    ...
    def check_filesystem(self):
        ...
        self.add_result(
            ...
            "可运行 'ppc8 config init' 创建配置目录" if not config_exists else None
        )
        ...
        self.add_result(
            ...
            "可运行 'ppc8 config init' 创建配置文件" if not config_file_exists else None
        )
    ...
    def check_config(self):
        ...
        try:
            ...
            self.add_result(
                ...
                config_detail,
                CheckIcons.SUCCESS,
                "运行 'ppc8 config init' 初始化配置" if not config_ok else None
            )
            ...
            if config_ok:
                ...
                self.add_result(
                    ...
                    "运行 'ppc8 config set tts.voice <语音名>' 设置" if not voice_ok else None
                )
...
def handle_check(full: bool, export: Optional[str] = None):
    ...
    # Windows 终端兼容：使用文本图标替代 emoji
    is_windows = sys.platform == "win32"
    gear_icon = "⚙" if not is_windows else "[GEAR]"
    
    output.title(f"{gear_icon} PPC8 系统诊断")
...
def handle_voices():
    """处理 voices 命令 - 列出所有可用语音"""
    output = OutputFormatter(verbose=False)
    
    output.show_banner()
    
    output.console.print(f"\n[bold {BrandColors.PRIMARY}]{'═' * 60}[/bold {BrandColors.PRIMARY}]")
    output.console.print(f"[bold white]  {Icons.MICROPHONE} PPC8 可用语音列表[/bold white]")
    output.console.print(f"[bold {BrandColors.PRIMARY}]{'═' * 60}[/bold {BrandColors.PRIMARY}]\n")
    ...
    try:
        ...
        output.console.print(f"\n[dim]提示：使用 'ppc8 config set --key tts.voice --value <语音 ID>' 设置默认语音[/dim]")
        output.console.print(f"[dim]示例：ppc8 config set --key tts.voice --value zh-CN-XiaoxiaoNeural[/dim]\n")
