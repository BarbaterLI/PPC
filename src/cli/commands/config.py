"""config 指令实现 - 冰璃岩开发组 (BLY Team)

配置管理功能，支持查看、修改、导入导出配置，以及交互式配置向导。
"""
...
def handle_config(
    ...
    参数:
        action: 操作类型 (show/get/set/reset/export/import/init/path/wizard)
        key: 配置键
        value: 配置值
        preset: 预设名称
        temp: 是否临时设置
    ...
    output.console.print(f"[bold white]  {Icons.GEAR} PPC8 配置管理[/bold white]")
    ...
    if action == "show":
        ...
        if key:
            ...
            output.error_panel(
                ...
                suggestion="使用 'ppc8 config get --key <配置键>' 获取配置值"
            )
            sys.exit(1)
        ...
        if key and value:
            ...
            output.error_panel(
                ...
                title="参数错误",
                error_type="ValueError",
                suggestion="使用 'ppc8 config set --key <配置键> --value <值>' 设置配置"
            )
            sys.exit(1)
        ...
        if export_path:
            ...
            output.error_panel(
                ...
                title="参数错误",
                error_type="ValueError",
                suggestion="使用 'ppc8 config export --export <路径>' 导出配置"
            )
            sys.exit(1)
        ...
        if import_path:
            ...
            output.error_panel(
                ...
                title="参数错误",
                error_type="ValueError",
                suggestion="使用 'ppc8 config import --import <路径>' 导入配置"
            )
            sys.exit(1)
        ...
        if config_manager.config_path.exists():
            ...
            output.console.print(f"\n[dim]提示：使用 'ppc8 config show' 查看完整配置[/dim]")
