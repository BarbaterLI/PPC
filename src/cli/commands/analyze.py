...
def _severity_icon(severity: Severity) -> str:
    if severity == Severity.CRITICAL:
        ...
        return "!"
    elif severity == Severity.MEDIUM or severity == Severity.LOW:
        return "i"
    return "i"
...
def _display_issues(
    ...
    output.print_table(headers, rows, title="问题列表")
...
def _display_health_score(
    ...
    content_lines = [
        f"{AnalyzeIcons.HEALTH} 健康评分: {score}/100",
        bar,
        f"评价: {label}",
    ]
    output.print_panel("\n".join(content_lines), title="健康度汇总", style=style)
        ...
        try:
            ...
            if "concurrency" in (issue.location or "") and "timeout" in (issue.location or ""):
                try:
                    ...
                    output.success("已修复: 超时时间调整为 120s")
                    fixed += 1
                except Exception as e:
                    output.error_text(f"修复失败: {e}")
                continue
            ...
            if "retries" in (issue.location or "") and "auto_retry" in (issue.location or ""):
                try:
                    ...
                    output.success("已修复: retries 设置为 3")
                    fixed += 1
                except Exception as e:
                    output.error_text(f"修复失败: {e}")
                continue
            ...
            if "rate_limit" in (issue.location or "") and "buffer_size" in (issue.location or ""):
                try:
                    ...
                    output.success("已修复: buffer_size 设置为 32")
                    fixed += 1
                except Exception as e:
                    output.error_text(f"修复失败: {e}")
                continue

            if "tts.retries" in (issue.location or "") and "reliability.tts_retry.max_retries" in (
                issue.location or ""
            ):
                try:
...
def _display_diff(
    ...
    label_current, _ = _score_label(current.score)
    label_prev, _ = _score_label(previous.score)

    content_lines = [
        f"较上次分析: {diff.score_diff:+.0f} 分"
        f" (当前 {current.score}/100 - {label_current}, "
        f"上次 {previous.score}/100 - {label_prev})",
    ]

    if diff.new_issues:
        content_lines.append("")
        content_lines.append(f"  新增 ({len(diff.new_issues)} 项)")
        for issue in diff.new_issues:
            content_lines.append(f"    - {issue.description}")

    if diff.fixed_issues:
        content_lines.append("")
        content_lines.append(f"  已修复 ({len(diff.fixed_issues)} 项)")
        for issue in diff.fixed_issues:
            content_lines.append(f"    - {issue.description}")

    if diff.persistent_issues:
        content_lines.append("")
        content_lines.append(f"  持续存在 ({len(diff.persistent_issues)} 项)")
        for issue in diff.persistent_issues:
            content_lines.append(f"    - {issue.description}")

    output.print_panel("\n".join(content_lines), title="差异对比", style="accent")
...
def _export_html_report(
    ...
    report: HealthReport,
    export_html: str,
    previous: HealthReport | None = None,
) -> None:
    try:
...
def _display_results(
    ...
    output.print_panel("PPC10 深度分析结果", title="深度分析", style="primary")
    ...
    if report.issues:
        ...
        for category, issues in grouped.items():
            ...
            output.info(f"{icon} {label} ({len(issues)} 项)")
...
def _apply_fixes_interactive(
    ...
    output.print_panel("自动修复", title="自动修复", style="accent")
    ...
    try:
        if output.console.is_terminal:
            if Confirm.ask(
                ...
                output.success(f"完成修复: {fixed_count} 项")
            else:
                output.info("已跳过自动修复")
        else:
            fixed_count = _apply_fixes(output, report.issues)
...
def _handle_watch_mode(
    ...
    def _make_watch_content(
        current_report: HealthReport,
        prev: HealthReport | None,
        cycle: int,
    ) -> str:
        label, _ = _score_label(current_report.score)

        lines = [
            f"监控周期 #{cycle}",
            f"健康评分: {current_report.score}/100 ({label})",
            f"问题总数: {len(current_report.issues)}",
        ]

        if prev is not None:
            diff = compute_diff(current_report, prev)
            lines.append(f"较上次变化: {diff.score_diff:+.0f} 分")
            if diff.new_issues:
                lines.append(f"新增: {len(diff.new_issues)} 项")
            if diff.fixed_issues:
                lines.append(f"修复: {len(diff.fixed_issues)} 项")
    ...
    try:
        ...
        while True:
            ...
            if first_run:
                ...
                output.info("初始分析完成，开始持续监控...")

            label, style = _score_label(report.score)
            content = _make_watch_content(report, previous_report, cycle)
            if output.mode == "human":
                output.console.clear()
            output.print_panel(content, title="实时监控", style=style)
...
class _SystemChecker:
    ...
    def add_result(
        ...
        if not status and suggestion:
            key = f"{category}:{name}"
            self.fix_suggestions.setdefault(key, [])
            self.fix_suggestions[key].append(suggestion)

    def check_system_environment(self):
        self.output.print_panel("系统环境检查", title="系统环境", style="info")
    ...
    def check_dependencies(self):
        self.output.print_panel("依赖包检查", title="依赖包", style="info")
    ...
    def check_network_connectivity(self):
        self.output.print_panel("网络连通性检查", title="网络连通性", style="info")
    ...
    def check_filesystem(self):
        self.output.print_panel("文件系统检查", title="文件系统", style="info")
    ...
    def check_system_resources(self):
        self.output.print_panel("系统资源检查", title="系统资源", style="info")
    ...
    def check_config(self):
        self.output.print_panel("配置验证", title="配置验证", style="info")
    ...
    def get_all_results(self) -> dict[str, Any]:
        ...
        return {
            ...
            "categories": dict(self.results.items()),
...
def _run_health_check(
    ...
    """运行轻量级健康检查，返回结果字典。"""
    output.print_panel("PPC10 系统健康检查", title="健康检查", style="primary")
    ...
    output.print_panel("检查结果汇总", title="检查结果汇总", style="primary")
    ...
    if pass_rate == 100:
        ...
        summary_icon = AnalyzeIcons.SUCCESS
        summary_text = "优秀"
    elif pass_rate >= 70:
        summary_style = "warning"
        summary_icon = AnalyzeIcons.WARNING
        summary_text = "良好"
    else:
        summary_style = "error"
        summary_icon = AnalyzeIcons.ERROR
        summary_text = "需改进"
    ...
    summary_content = (
        ...
        f"失败: {summary['failed']}  "
        f"通过率: {pass_rate:.1f}%  "
        f"{summary_icon} 评价：{summary_text}"
    )
    output.print_panel(summary_content, title="检查汇总", style=summary_style)

    if checker.fix_suggestions:
        output.print_panel("修复建议", title="修复建议", style="accent")
        for key, suggestion in checker.fix_suggestions.items():
            category, name = key.split(":", 1)
            output.info(f"{name}: {suggestion}")
...
def _apply_health_fixes(
    ...
    fixed_count = 0
    for key, _suggestion in checker.fix_suggestions.items():
        category, name = key.split(":", 1)
        ...
        if name == "配置目录":
            ...
            try:
                ...
                output.error_text(f"创建配置目录失败：{e}")
            ...
            try:
                ...
                output.error_text(f"创建配置文件失败：{e}")
            ...
            try:
                ...
                output.error_text(f"创建输出目录失败：{e}")
...
def handle_analyze(
    ...
    deep_flags_set = any([performance, config, errors, dependency, network, resource, quality, diff, watch])
    ...
    output.print_panel("PPC10 系统深度分析", title="深度分析", style="primary")
    ...
    output.info(f"分析模块: {' | '.join(categories)}")
