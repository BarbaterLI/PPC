...
class AudioProcessor:
    ...
    def validate(self, file_path: str | Path) -> tuple[bool, str | None]:
                    ...
                    try:
                        with wave.open(str(file_path), "rb") as wf:
                            ...
                            if wf.getparams()[:3] != params[:3]:
                                logger.warning(f"音频参数不匹配 ({file_path.name})，将重新对齐")
        ...
        *,
        ...
        try:
            with wave.open(str(path), "rb") as wf:
                ...
                min_dur_ms = min_duration_ms if min_duration_ms is not None else self.silence_min_duration_ms
    ...
    def _compute_rms(raw: bytes, sample_width: int, n_channels: int) -> float:
        ...
        try:
            ...
            if sample_width == 1:
                samples = [(b - 128) for b in raw[: n_samples * sample_width]]
            elif sample_width == 2:
    ...
    def _normalize_rms_fallback(self, file_path: Path, output_path: Path) -> bool:
