...
class CircuitBreaker:
    ...
    def __init__(
        ...
        on_state_change: Callable[["CircuitBreaker", CircuitState, CircuitState], None] | None = None,
    ):
        self.name = name
    ...
    async def _gate(self) -> tuple[bool, bool]:
        ...
        async with self._lock:
            ...
            if self._state == CircuitState.HALF_OPEN:
                canary_window = max(
                    1,
                    int(round(self._half_open_limiter._max_calls * self._half_open_limiter._canary_ratio)),
                )
...
async def get_circuit_breaker(name: str, config: CircuitBreakerConfig | None = None) -> CircuitBreaker:
