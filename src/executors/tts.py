...
def _add_batch_with_progress(
    self, input_dir, output_dir, progress_handler=None, voice=None, pattern="*.txt", recursive=False
):
    return add_batch_with_progress(self, input_dir, output_dir, progress_handler, voice, pattern, recursive)


TTSExecutor.add_batch = _add_batch  # type: ignore[attr-defined]  # 运行时动态挂载方法以兼容旧 API
TTSExecutor.add_batch_with_progress = _add_batch_with_progress  # type: ignore[attr-defined]  # 运行时动态挂载方法以兼容旧 API
