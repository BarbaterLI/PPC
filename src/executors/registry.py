...
class ExecutorRegistry:
    ...
    def register(
        ...
        config_schema: dict[str, Any] | None = None,
        factory: Callable[..., BaseExecutor] | None = None,
        force: bool = False,
    ) -> None:
        """Register an executor with the registry."""
        if not issubclass(executor_class, BaseExecutor):
            raise TypeError(f"Executor class must be a subclass of BaseExecutor, got {executor_class}")
    ...
    def get(self, name: str) -> ExecutorMetadata | None:
    ...
    def get_by_tag(self, tag: str) -> list[ExecutorMetadata]:
        ...
        return [meta for meta in self._executors.values() if tag in meta.tags]
