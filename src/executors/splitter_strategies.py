...
from ..config.schema import ConditionType, RuleType
...
def _merge_patterns(executor, preset: str) -> list[tuple]:
    ...
    patterns = executor._chapter_patterns.get(preset, executor._chapter_patterns["default"])
...
def _detect_volumes(executor, content: str) -> list[VolumeInfo]:
    ...
    volumes = []
    current_volume: VolumeInfo | None = None
    volume_index = 0
    ...
    for i, line in enumerate(lines):
        ...
        if is_volume_match:
            ...
            current_volume = VolumeInfo(
                index=volume_index, title=safe_title, start_line=i, end_line=len(lines), chapters=[]
            )
...
def _validate_title(title: str, rule: Any | None = None) -> bool:
    """验证标题是否符合规则"""
    if not rule:
        return True
...
def _extract_title(text: str, rule: Any | None = None) -> str:
    """从匹配文本中提取标题"""
    if not rule:
        return text.strip()
...
def _check_excluded_patterns(text: str, excluded_patterns: list[str]) -> bool:
    """检查是否匹配排除模式"""
    stripped = text.strip()
    return any(re.search(pattern, stripped, re.IGNORECASE) for pattern in excluded_patterns)
...
def _check_conditions(lines: list[str], line_index: int, rule) -> bool:
    ...
    for i in range(1, len(results)):
        ...
        final_result = final_result or result if logic == "or" else final_result and result
...
def _split_flat(executor, content: str) -> list[ChapterInfo]:
    ...
    for i, line in enumerate(lines):
        ...
        if match_found:
            ...
            if current_chapter:
                ...
                finalized = _finalize_chapter(executor, current_chapter, lines, split_config.min_chapter_length)
                if finalized:
            ...
            current_chapter = ChapterInfo(
                index=chapter_index, title=extracted_title, start_line=i, end_line=len(lines), content=""
            )

            if matched_rule and matched_rule.stop_on_match:
                break

            skip_lines = max(matched_rule.skip_lines_after_match, matched_rule.consume_lines - 1) if matched_rule else 0
    ...
    if not chapters and lines:
        chapters.append(
            ChapterInfo(index=1, title="全文", start_line=0, end_line=len(lines), content="".join(lines).strip())
        )

    return chapters


def _try_match_line(executor, line: str, line_index: int, lines: list[str]) -> tuple[bool, Any | None]:
...
def _check_rule_match(executor, line: str, line_index: int, lines: list[str], rule) -> bool:
    ...
    return _validate_title(stripped, rule)


def _finalize_chapter(executor, chapter: ChapterInfo, lines: list[str], min_length: int) -> ChapterInfo | None:
    """完成当前章节，检查长度后返回"""
    ...
    if chapter.content:
        logger.warning(
            "章节 '%s' 内容长度 %d 小于最小长度 %d，将被丢弃", chapter.title, len(chapter.content), min_length
        )
...
def _format_volume_dir(executor, volume_title: str, volume_index: int) -> str:
    """格式化卷目录名"""
    fmt: str = executor.config.split.volume_dir_prefix
...
async def split_directory(executor, input_dir: Path, output_dir: Path, pattern: str = "*.txt"):
    """批量分割目录下的文件"""
    ...
    metrics = ExecutionMetrics(duration=time.time() - start_time, bytes_processed=succeeded, request_count=failed)
    ...
    if failed == 0:
        ...
        return ExecutionResult.fail(error="所有文件分割失败", error_code=ErrorCodes.BATCH_PROCESSING_FAILED.value)
