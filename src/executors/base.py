...
class BaseExecutor(ABC, Generic[InputType, OutputType]):
    ...
    def __init__(
        ...
        self.config = config or PPC8Config()
...
class BatchExecutor(BaseExecutor[Path, Any]):
    ...
    async def process_batch(
        ...
        for input_path in input_paths:
            ...
            progress_interval = self.config.core.progress_interval
            if self._progress_callback and processed % progress_interval == 0:
...
class StreamingExecutor(BaseExecutor[str, bytes]):
    ...
    def __init__(
        ...
        self._buffer: list = []
        self._buffer_size = 0
        self._flush_threshold = self.config.performance.stream_flush_threshold
