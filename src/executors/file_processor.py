...
from src.utils.core import MemoryMonitor, detect_encoding, memory_efficient, sanitize_filename
...
class FileCache:
    ...
    def get(self, file_path: Path, encoding: str = "utf-8") -> str | None:
...
class TextSegmenter:
    ...
    def __init__(self, max_length: int = 2500, min_length: int = 100, punctuations: list[str] | None = None):
    ...
    def _find_best_split_point(self, text: str, start: int, end: int) -> int:
        ...
        for i in range(end - 1, search_range - 1, -1):
            if text[i] in self.punctuations and i > start + self.min_length:
                j = i + 1
                while j < end and text[j] in " \t\n":
                    j += 1
                best_split = j if j > i + 1 else i + 1
                break
...
class FileProcessor:
    ...
    def __init__(self, config: dict[str, Any], memory_monitor: MemoryMonitor | None = None):
        self.config = config
        self.memory_monitor = memory_monitor or MemoryMonitor()
        self.file_cache = FileCache(max_size=config.get("performance", {}).get("max_file_cache_size", 150))
    ...
    def detect_encoding(self, file_path: Path) -> str | None:
        encodings = self.config.get("split", {}).get("encoding_fallback", ["utf-8", "gbk", "gb2312", "utf-16"])
    ...
    @memory_efficient
    def read_file_cached(self, file_path: Path, encoding: str = "utf-8") -> str | None:
    ...
    def is_epub_file(self, file_path: Path) -> bool:
        ...
        try:
            with zipfile.ZipFile(file_path, "r") as zip_file:
                return (
                    "mimetype" in zip_file.namelist()
                    and zip_file.read("mimetype").decode("utf-8").strip() == "application/epub+zip"
                )
        except Exception:
            return False
