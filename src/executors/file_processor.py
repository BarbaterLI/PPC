...
from src.utils.core import MemoryMonitor, memory_efficient
...
class TextSegmenter:
    ...
    def __init__(self, max_length: int = 2500, min_length: int = 100, punctuations: List[str] = None):
...
class FileProcessor:
    def __init__(self, config: Dict[str, Any], memory_monitor: MemoryMonitor = None):
        self.config = config
    ...
    def is_epub_file(self, file_path: Path) -> bool:
        ...
        try:
            with zipfile.ZipFile(file_path, 'r') as zip_file:
                return 'mimetype' in zip_file.namelist() and \
