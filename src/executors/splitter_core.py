...
from ..utils.core import detect_encoding, sanitize_filename
...
class ChapterInfo:
    ...
    end_line: int
    content: str
    _volume_index: int | None = None
    _volume_title: str | None = None
...
class SplitterExecutor(BaseExecutor):
    ...
    def __init__(self, config: PPC10Config | None = None, custom_rules: list | None = None):
        ...
        self._current_volumes: list[VolumeInfo] | None = None
    ...
    async def execute(self, input_path: Path, output_dir: Path) -> ExecutionResult[list[Path]]:
        """执行分割任务"""
            ...
            stats.append(
                {
                    "index": vol.index,
                    "title": vol.title,
                    "chapter_count": len(vol.chapters or []),
                }
            )
        return stats
...
async def split_file(executor, input_path: Path, output_dir: Path) -> ExecutionResult[list[Path]]:
    """便捷分割接口"""
    ...
    try:
        if not input_path.exists():
            return ExecutionResult.fail(
                error=f"输入文件不存在 {input_path}", error_code=ErrorCodes.FILE_NOT_FOUND.value
            )
        ...
        metrics = ExecutionMetrics(
            duration=time.time() - start_time, bytes_processed=len(output_files), request_count=len(output_files)
        )
...
def _generate_output_name(executor, output_dir: Path, index: int, title: str) -> Path:
    """生成输出文件名"""
