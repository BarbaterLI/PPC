...
def test_concurrency_limited_by_api_concurrency(tmp_path):
    ...
    assert max_active >= 2, f"应达到 api_concurrency=2 上限"
...
def test_segment_level_progress_reported(tmp_path):
    ...
    _run(engine._merge_segments(
        segments, output, 0.0, disable_timeout=True,
...
def test_segment_hard_timeout_triggers(tmp_path):
    """段级硬超时：单段 synthesize 卡死时，asyncio.wait_for 触发 TimeoutError，转 fail。"""
    import time
    engine = _make_engine()
    output = tmp_path / "out.mp3"
    segments = _setup_segments(output, n=2)
    ...
    assert elapsed < 3.0, f"硬超时未生效，耗时 {elapsed:.1f}s"


def test_segment_hard_timeout_uses_calculate_timeout(tmp_path):
    """段级硬超时：使用 _calculate_timeout(segment) 动态计算，而不是固定值。"""
    engine = _make_engine()
    output = tmp_path / "out.mp3"
