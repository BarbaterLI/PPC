...
class TestTTSNode:
    ...
    def test_constructor_custom(self):
        cfg = MagicMock()
        node = TTSNode(host="127.0.0.1", port=9000, max_concurrency=8, config=cfg, node_id="alpha")
...
class TestNodeClient:
    def test_constructor_stores_config(self):
        ...
        with patch("src.distributed.node_adapter.aiohttp"):
    ...
    def test_session_reuse(self):
        ...
        with patch("src.distributed.node_adapter.aiohttp") as mock_aio:
    ...
    def test_heartbeat_task_starts_and_stops(self):
        cfg = NodeClientConfig(
            base_url="http://x:1", enable_heartbeat=True, heartbeat_interval=0.05, heartbeat_timeout=0.1
        )
        with patch("src.distributed.node_adapter.aiohttp") as mock_aio:
            mock_aio.TCPConnector.return_value = MagicMock()
            mock_aio.ClientSession.return_value = MagicMock()
