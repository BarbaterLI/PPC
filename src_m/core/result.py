from typing import Any, Callable, Generic, TypeVar, Union, Optional, List, Dict
...
class ExecutionMetrics:
    __slots__ = (
        "start_time", "end_time", "duration", "memory_peak_kb",
        "bytes_processed", "request_count", "retry_count", "extra",
    )
...
class Result(Generic[T]):
    def __init__(
        ...
        self.partial = state == ResultState.PARTIAL
    ...
    @classmethod
    def ok(cls, value: Optional[T] = None, metrics: Optional[ExecutionMetrics] = None) -> "Result[T]":
        return cls(state=ResultState.SUCCESS, value=value, metrics=metrics)

    @classmethod
    def fail(cls, error: str = "", error_code: Optional[str] = None,
    ...
    def map(self, func: Callable[[T], Any]) -> "Result[Any]":
        if self.state != ResultState.FAILURE:
            return Result.ok(func(self.value), self.metrics)
