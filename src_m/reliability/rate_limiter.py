...
class TwoTierRateLimiter:
    ...
    def __init__(self, config: Optional[TierRateLimiterConfig] = None):
        self._config = config or TierRateLimiterConfig()
        self._global = self._build(self._config.global_strategy, scope="global",
    ...
    @staticmethod
    def _build(strategy: RateLimitStrategy, *, scope: str,
               max_tokens: int, refill_rate: float,
               max_requests: int, window_size: float):
    ...
    async def acquire(self, node_id: str, tokens: int = 1) -> RateLimitResult:
        ...
        if not global_result.allowed:
            return RateLimitResult(
                allowed=False, wait_time=global_result.wait_time,
                current_rate=global_result.current_rate, scope="global",
            )
        ...
        if not node_result.allowed:
            return RateLimitResult(
                allowed=False, wait_time=node_result.wait_time,
                current_rate=node_result.current_rate, scope="node",
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        return {
            ...
            "nodes": {
                ...
                for node_id, l in self._node_limiters.items()
            },
        }
    ...
    def reset(self) -> None:
        ...
        for l in self._node_limiters.values():
            l.allowed = 0
            l.rejected = 0
