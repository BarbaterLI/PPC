...
class CustomRule(BaseModel):
    ...
    # 标题提取
    title_pattern: Optional[str] = Field(None, description="标题提取正则模式")
    title_group: int = Field(default=0, description="标题提取的捕获组号")
    title_prefix_remove: Optional[str] = Field(None, description="标题前缀移除模式")
    title_suffix_remove: Optional[str] = Field(None, description="标题后缀移除模式")
    ...
    # 行号范围
    line_range_start: Optional[int] = Field(None, ge=0, description="起始行号（0 表示不限）")
    line_range_end: Optional[int] = Field(None, ge=0, description="结束行号（None 表示不限）")
    ...
    volume_dir_format: Optional[str] = Field(None, description="卷目录名格式（支持 {title} {index}）")
    ...
    @classmethod
    def validate_title_pattern(cls, v: Optional[str]) -> Optional[str]:
        """验证标题提取正则模式"""
        import re
        if v is not None:
            try:
                ...
                raise ValueError(f"无效的标题提取正则表达式: {e}")
        return v

    @field_validator('title_prefix_remove')
    @classmethod
    def validate_title_prefix_remove(cls, v: Optional[str]) -> Optional[str]:
        """验证标题前缀移除模式"""
        import re
        if v is not None:
            try:
                ...
                raise ValueError(f"无效的标题前缀移除正则表达式: {e}")
        return v

    @field_validator('title_suffix_remove')
    @classmethod
    def validate_title_suffix_remove(cls, v: Optional[str]) -> Optional[str]:
        """验证标题后缀移除模式"""
        import re
        if v is not None:
            try:
                ...
                raise ValueError(f"无效的标题后缀移除正则表达式: {e}")
        return v
...
class TTSConfig(BaseModel):
    ...
    api_concurrency: Optional[int] = Field(default=None, ge=1, le=64, description="API 并发数（控制同时调用 Edge TTS API 的最大并发数，None 时自动推导）")
    ramp_up_enabled: bool = Field(default=False, description="启用并发渐进预热（从1并发逐步增加到设定并发，规避风控）")
    ramp_up_duration: float = Field(default=30.0, ge=5.0, le=300.0, description="并发预热持续时间（秒），在此时间内从1逐步增加到设定并发数")
    ...
    def validate_cross_fields(self) -> 'TTSConfig':
        ...
        if self.min_segment_length > self.max_segment_length:
            raise ValueError(f"min_segment_length ({self.min_segment_length}) 不能大于 max_segment_length ({self.max_segment_length})")
...
class ExtensionConfig(BaseModel):
    ...
    extension_dirs: List[str] = Field(
        default_factory=lambda: ["extensions"],
        description="扩展目录列表（相对路径或绝对路径）"
    )
...
class OutputConfig(BaseModel):
    ...
    post_processing: List[str] = Field(
        default_factory=list,
        description="后处理管道（如 reverb, compression, equalizer）"
    )
...
class WebhookConfig(BaseModel):
    ...
    events: List[WebhookEventType] = Field(
        default_factory=lambda: [WebhookEventType.TASK_COMPLETED],
        description="触发 Webhook 的事件类型"
    )
    ...
    secret: Optional[str] = Field(default=None, description="签名密钥（用于 HMAC 签名验证）")
...
class DistributedConfig(BaseModel):
    ...
    load_balance_strategy: str = Field(default="round_robin", description="负载均衡策略: round_robin | least_connections | best_response_time")
    ...
    adaptive_load_balance: AdaptiveLoadBalanceConfig = Field(default_factory=AdaptiveLoadBalanceConfig, description="自适应负载均衡配置")
