...
def list_extensions():
    try:
        ...
        return jsonify({"error": str(e), "code": "EXT_LIST_ERROR"}), 500
...
def get_webui_configs():
    try:
        ...
        return jsonify({"error": str(e), "code": "WEBUI_CONFIG_ERROR"}), 500
    ...
    try:
        ...
        try:
            ...
            if result["success"]:
                ...
                return jsonify({"error": result.get("error", "Install failed"), "code": "INSTALL_FAILED"}), 500
        finally:
            tmp_path.unlink(missing_ok=True)

    except Exception as e:
        logger.exception("Failed to install extension")
        return jsonify({"error": str(e), "code": "EXT_INSTALL_ERROR"}), 500
...
def get_extension(name: str):
    try:
        ...
        if info is None:
            return jsonify({"error": f"Extension '{name}' not found", "code": "NOT_FOUND"}), 404

        return jsonify(info)
    except Exception as e:
        logger.exception("Failed to get extension info")
        return jsonify({"error": str(e), "code": "EXT_GET_ERROR"}), 500
...
def uninstall_extension(name: str):
    try:
        ...
        if result["success"]:
            ...
            return jsonify({"error": result.get("error", "Uninstall failed"), "code": "UNINSTALL_FAILED"}), 500

    except Exception as e:
        logger.exception("Failed to uninstall extension")
        return jsonify({"error": str(e), "code": "EXT_UNINSTALL_ERROR"}), 500
...
def enable_extension(name: str):
    try:
        ...
        if success:
            ...
            return jsonify({"error": f"Extension '{name}' not found or already enabled", "code": "ENABLE_FAILED"}), 400

    except Exception as e:
        logger.exception("Failed to enable extension")
        return jsonify({"error": str(e), "code": "EXT_ENABLE_ERROR"}), 500
...
def disable_extension(name: str):
    try:
        ...
        if success:
            ...
            return jsonify({"error": f"Extension '{name}' not found or already disabled", "code": "DISABLE_FAILED"}), 400

    except Exception as e:
        logger.exception("Failed to disable extension")
        return jsonify({"error": str(e), "code": "EXT_DISABLE_ERROR"}), 500
