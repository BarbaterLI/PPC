        ...
        if checkpoint_data:
            ...
            for task_id, checkpoint_task in checkpoint_data.tasks.items():
                ...
                if task.status == "pending":
                    ...
                    if progress_handler:
                        input_path = Path(task.input_file) if isinstance(task.input_file, (str, Path)) else task.input_file
                        progress_handler.register_task(task_id, input_path.name)

            resumed_from_checkpoint = True
            logger.info("已恢复 %d 个任务，其中 %d 个已完成", len(executor._tasks), len(executor._tasks) - executor._task_queue.qsize())

    if not resumed_from_checkpoint:
        if recursive:
            txt_files = sorted(input_dir.rglob(pattern))
        else:
        ...
        for txt_file in txt_files:
            ...
            task_id = hashlib.md5(
                f"{txt_file.resolve()}{voice}".encode()
            ).hexdigest()[:16]
        ...
        if executor._checkpoint_manager and executor._tasks:
            executor._checkpoint_manager.create_checkpoint(
                input_dir, output_dir, voice, executor._tasks
            )
    ...
    if executor._workers:
        try:
            await asyncio.wait_for(
                asyncio.gather(*executor._workers, return_exceptions=True),
                timeout=5.0
            )
...
async def _start_processing(executor, progress_handler: Optional[Any] = None):
...
async def _worker_loop(executor, worker_id: str, progress_handler: Optional[Any] = None):
    try:
        while True:
...
async def _get_next_task(executor) -> Optional[TTSTask]:
    try:
...
async def _process_single_task(executor, task: TTSTask, worker_id: str, progress_handler: Optional[Any] = None):
    try:
...
async def _execute_task_with_retry(executor, task: TTSTask, worker_id: str, progress_handler: Optional[Any] = None):
    from ..reliability.retry import RetryConfig as _RetryConfig, _calculate_delay
    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            ...
            if attempt < max_retries:
                ...
                if "超时" in error_msg or "timeout" in error_msg.lower():
                    logger.warning(
                        ...
                        delay, attempt + 1, max_retries + 1, error_msg
                    )
                else:
                    logger.warning(
                        "任务失败，%.1fs 后重试 (%d/%d)：%s",
            ...
            _handle_task_failure(
                executor, task, f"重试 {max_retries} 次后仍失败: {result.error}",
                progress_handler, attempt
            )
            return
            ...
            if attempt < max_retries:
                ...
                if is_timeout:
                    logger.warning(
                        "任务异常，%.1fs 后重试 (%d/%d)：%s\n  提示：文本过长或服务器繁忙，建议降低并发数(--concurrency)或音频速度(--rate)",
                        delay, attempt + 1, max_retries + 1, e
                    )
                else:
                    logger.warning(
                        "任务异常，%.1fs 后重试 (%d/%d)：%s",
