...
class EPUBEngine(BaseEngine[Path, Dict[str, Any]]):
    ...
    def _is_valid_epub(file_path: Path) -> bool:
        ...
        try:
            with zipfile.ZipFile(file_path, "r") as zf:
                return (
                    MIMETYPE in zf.namelist()
                    and zf.read(MIMETYPE).decode(ENCODING_UTF8).strip() == MIMETYPE
                )
        except Exception:
            return False
    ...
    @staticmethod
    def _get_opf_path(zf: zipfile.ZipFile) -> Optional[str]:
        ...
        return rootfile.get("full-path", "") if rootfile else None

    def _parse_opf_metadata(
        self, zf: zipfile.ZipFile, opf_path: str, metadata: EPUBMetadata
    ) -> None:
        """解析 OPF 文件中的元数据"""
        try:
    ...
    def _parse_html_chapter(
        self, item_id: str, html_content: str, order: int
    ...
    async def extract_text_only(
        self, epub_path: Path, output_file: Path
