...
fanqie_app = typer.Typer(
    name="fanqie",
    help="番茄小说下载器管理（安装/更新/启动TUI/启动Server）",
...
def _get_extension():
    ...
    from src_m.cli.typer_app import get_extension_loader
    loader = get_extension_loader()
    ext = loader.get_extension("fanqie_downloader")
    if ext is None:
        from src_m.extensions.fanqie.extension import extension
        ext = extension
    return ext


@fanqie_app.command("install")
def fanqie_install(
    ...
    musl: bool = typer.Option(False, "--musl", help="优先下载 musl 版本（适用于 NAS/软路由）"),
):
    """安装番茄小说下载器

    从 GitHub Releases 下载适合当前系统的番茄小说下载器，
    默认使用镜像源加速，安装到项目 fanqie/ 目录下。

    示例:
        ppc9 fanqie install
        ppc9 fanqie install --no-mirror
        ppc9 fanqie install --musl
    """
    ext = _get_extension()
    ...
    if ext.is_installed():
        ...
        console.print(
            ...
            f"如需更新请使用: ppc9 fanqie update[/{BrandColors.WARNING}]"
        )
        raise typer.Exit()
    ...
    try:
        ...
        if result["success"]:
            ...
            console.print("  TUI 模式:   ppc9 fanqie tui")
            console.print("  Server 模式: ppc9 fanqie server")
        else:
            console.print(
        ...
        console.print()
        console.print(
            f"[{BrandColors.ERROR}]{Icons.ERROR} 安装异常: {e}[/{BrandColors.ERROR}]"
        )
        raise typer.Exit(1)


@fanqie_app.command("update")
def fanqie_update(
    mirror: bool = typer.Option(True, "--mirror/--no-mirror", help="使用镜像源加速下载"),
    mirror_host: str = typer.Option("gh.llkk.cc", "--mirror-host", help="镜像源地址"),
    ...
    检查 GitHub 最新版本并更新本地安装。如果已是最新版本则跳过，
    除非使用 --force 强制更新。

    示例:
        ppc9 fanqie update
        ppc9 fanqie update --force
        ppc9 fanqie update --no-mirror
    """
    ext = _get_extension()

    if not ext.is_installed():
        console.print(
            f"[{BrandColors.WARNING}]番茄小说下载器未安装，请先运行: ppc9 fanqie install[/{BrandColors.WARNING}]"
        )
        raise typer.Exit(1)
    ...
    try:
        ...
        console.print()
        console.print(
            f"[{BrandColors.ERROR}]{Icons.ERROR} 更新异常: {e}[/{BrandColors.ERROR}]"
        )
        raise typer.Exit(1)


@fanqie_app.command("tui")
def fanqie_tui(
    data_dir: Optional[str] = typer.Option(None, "--data-dir", "-d", help="数据目录（配置/日志存放位置）"),
):
    """启动番茄小说下载器 TUI 模式
    ...
    示例:
        ppc9 fanqie tui
        ppc9 fanqie tui --data-dir /path/to/data
    """
    ext = _get_extension()
    ...
    console.print(
        f"[{BrandColors.INFO}]{Icons.ROCKET} 正在启动番茄小说下载器 TUI...[/{BrandColors.INFO}]"
    )
    ...
    try:
        ...
        console.print(f"\n[{BrandColors.INFO}]已退出 TUI 模式[/{BrandColors.INFO}]")
    except FileNotFoundError as e:
        console.print(f"[{BrandColors.ERROR}]{Icons.ERROR} {e}[/{BrandColors.ERROR}]")
        raise typer.Exit(1)


@fanqie_app.command("server")
def fanqie_server(
    host: str = typer.Option("127.0.0.1", "--host", "-h", help="监听地址"),
    port: int = typer.Option(18423, "--port", "-p", help="监听端口"),
    password: Optional[str] = typer.Option(None, "--password", "-P", help="Web UI 访问密码"),
    ...
    示例:
        ppc9 fanqie server
        ppc9 fanqie server --host 0.0.0.0 --port 8080
        ppc9 fanqie server --password mypass
        ppc9 fanqie server --data-dir /path/to/data
    """
    import webbrowser
    ...
    console.print(
        f"[{BrandColors.INFO}]{Icons.ROCKET} 正在启动番茄小说下载器 Server...[/{BrandColors.INFO}]"
    )
...
def fanqie_status():
    ...
    console.print(f"[bold]番茄小说下载器 状态[/bold]\n")
    ...
    if status["installed"]:
        ...
        if update_available is True:
            console.print(
                f"  最新版本: [{BrandColors.WARNING}]{latest} (有更新)[/{BrandColors.WARNING}]"
            )
            console.print(f"  更新命令: ppc9 fanqie update")
        ...
        console.print(f"  安装命令: ppc9 fanqie install")


@fanqie_app.command("config")
def fanqie_config(
    action: str = typer.Argument("show", help="操作: show/get/set/path/edit"),
    key: Optional[str] = typer.Option(None, "--key", "-k", help="配置键（支持多级，如 save_path）"),
    value: Optional[str] = typer.Option(None, "--value", "-v", help="配置值"),
):
    """管理番茄小说下载器配置

    管理 fanqie/ 目录下的 config.yml，与 PPC9 的 config.yaml 完全隔离。

    可用操作:
      show  - 显示完整配置
      get   - 获取指定配置项的值（需 --key）
      set   - 设置配置项（需 --key 和 --value）
      path  - 显示配置文件路径
      edit  - 用系统编辑器打开配置文件

    示例:
        ppc9 fanqie config show
        ppc9 fanqie config get -k save_path
        ppc9 fanqie config set -k save_path -v /path/to/novels
        ppc9 fanqie config set -k enable_tts -v true
        ppc9 fanqie config path
        ppc9 fanqie config edit
    """
    from src_m.extensions.fanqie.downloader import (
        config_exists, read_config, write_config,
        get_config_value, set_config_value, _get_config_path,
    )
    ...
    if action == "path":
        ...
        if config_path.exists():
            ...
            console.print(f"文件状态: [{BrandColors.WARNING}]不存在（首次启动下载器后自动生成）[/{BrandColors.WARNING}]")
        return
    ...
    if action == "show":
        ...
        if content is None:
            config_path = _get_config_path()
            console.print(
                f"[{BrandColors.WARNING}]配置文件不存在: {config_path}[/{BrandColors.WARNING}]"
            )
            console.print("[dim]提示: 首次启动下载器后会自动生成配置文件[/dim]")
            console.print("[dim]      也可通过 ppc9 fanqie config set -k <key> -v <value> 创建[/dim]")
        else:
            console.print(f"[bold]番茄小说下载器 配置[/bold]")
    ...
    if action == "get":
        ...
        if val is None:
            console.print(f"[{BrandColors.WARNING}]配置项 '{key}' 不存在[/{BrandColors.WARNING}]")
        else:
    ...
    if action == "set":
        ...
        if set_config_value(key, value):
            ...
            console.print(
                f"[{BrandColors.ERROR}]{Icons.ERROR} 设置失败[/{BrandColors.ERROR}]"
            )
            raise typer.Exit(1)
        return
    ...
    if action == "edit":
        ...
        if not config_path.exists():
            console.print(
                f"[{BrandColors.WARNING}]配置文件不存在，将创建空配置[/{BrandColors.WARNING}]"
            )
    ...
    console.print(
        f"[{BrandColors.ERROR}]{Icons.ERROR} 未知操作: {action}[/{BrandColors.ERROR}]"
    )
    console.print("可用操作: show / get / set / path / edit")
    raise typer.Exit(1)


@fanqie_app.command("uninstall")
def fanqie_uninstall(
    ...
    示例:
        ppc9 fanqie uninstall
        ppc9 fanqie uninstall --yes
    """
    ext = _get_extension()

    if not ext.is_installed():
        console.print(
            f"[{BrandColors.WARNING}]番茄小说下载器未安装[/{BrandColors.WARNING}]"
        )
        raise typer.Exit()
    ...
    if ext.uninstall():
        console.print(
            f"[{BrandColors.SUCCESS}]{Icons.SUCCESS} 番茄小说下载器已卸载[/{BrandColors.SUCCESS}]"
        )
    else:
        console.print(
            f"[{BrandColors.ERROR}]{Icons.ERROR} 卸载失败[/{BrandColors.ERROR}]"
        )
        raise typer.Exit(1)
