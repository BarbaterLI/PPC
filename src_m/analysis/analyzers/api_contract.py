...
``Engine``/``Executor``/``Pipeline`` classes with the :mod:`inspect`
...
# Classes that we always look at when scanning for Engine/Executor/Pipeline
...
DEFAULT_TARGETS: List[Tuple[str, str]] = [
    ...
    ("src_m.engines.chapter_engine.ChapterEngine", "engine"),
    ("src_m.engines.epub_engine.EPUBEngine", "engine"),
    ("src_m.executors.base.BaseExecutor", "executor"),
    ("src_m.executors.tts.TTSExecutor", "executor"),
    ("src_m.executors.splitter.SplitterExecutor", "executor"),
    ("src_m.executors.batcher.BatcherExecutor", "executor"),
    ("src_m.pipeline.engine.PipelineEngine", "pipeline"),
    ("src_m.pipeline.builder.PipelineBuilder", "pipeline"),
    ("src_m.pipeline.validator.PipelineValidator", "pipeline"),
]
...
def _safe_import(dotted_path: str) -> Optional[Type[Any]]:
    ...
    for name in sorted(set(baseline) - set(current)):
        ...
        issues.append(
            AnalysisIssue(
                ...
                suggestion=(
                    "考虑在父类/兼容层保留该方法或明确记录到 CHANGELOG；"
                    "若确认移除，请在基线文件中同步更新"
                ),
    ...
    for name in sorted(set(current) & set(baseline)):
        ...
        if cur.params != base.params:
            if len(cur.params) < len(base.params):
                changes.append(
                    f"参数数量减少: {base.params} -> {cur.params}"
                )
            else:
                ...
                if added:
                    changes.append(
                        f"新增位置参数 {added}; 已有调用方传入的位置实参会被错位"
                    )
        ...
        if cur.return_annotation != base.return_annotation:
            changes.append(
                f"返回类型变化: {base.return_annotation} -> {cur.return_annotation}"
            )

        # Sync -> async or vice versa is breaking.
        if cur.is_async != base.is_async:
            changes.append(
                f"同步/异步签名变化: async={base.is_async} -> async={cur.is_async}"
            )
        ...
        if changes:
            issues.append(
                AnalysisIssue(
                    ...
                    category=AnalysisCategory.CODE_QUALITY,
                    description=(
                        f"[API 破坏] {class_id}.{name}() 签名变化: " + "; ".join(changes)
                    ),
                    suggestion=(
                        "保持向后兼容：新增参数应提供默认值；"
                        "如确需修改返回类型，请通过新增方法或兼容层过渡"
                    ),
                    location=f"{class_id}.{name}",
                    details={
...
class APIContractAnalyzer(BaseAnalyzer):
    """Analyzer for Engine/Executor/Pipeline public API breaking changes."""

    def __init__(
        self,
        baseline_path: Optional[Path] = None,
        targets: Optional[Iterable[Tuple[str, str]]] = None,
    ...
    async def analyze(self, context: Optional[Dict[str, Any]] = None) -> List[AnalysisIssue]:
        ...
        current_snapshot = {
            ...
            "classes": {
                dotted: _snapshot_class(dotted, kind)
                for dotted, kind in self._targets
            },
        }
        ...
        if is_first_run:
            if self._auto_create:
                ...
                return [
                    AnalysisIssue(
                        ...
                        category=AnalysisCategory.CODE_QUALITY,
                        description=(
                            "已生成 API 基线快照 (首次运行)，路径: "
                            f"{self._baseline_path}"
                        ),
                        suggestion=(
                            "下次执行将自动与该基线对比以检测破坏性变更"
                        ),
            ...
            return [
                AnalysisIssue(
                    ...
                    suggestion=(
                        f"使用 'ppc10 analyze --api-contract --api-baseline-init' "
                        "生成基线快照"
                    ),
                    location=str(self._baseline_path),
        ...
        for class_id, snap in current_snapshot["classes"].items():
            ...
            cur_methods: Dict[str, MethodSignature] = {
                n: MethodSignature.from_dict(n, m)
                for n, m in snap.get("methods", {}).items()
            }
            base_methods: Dict[str, MethodSignature] = {
                n: MethodSignature.from_dict(n, m)
                for n, m in base_class.get("methods", {}).items()
            }
