...
defaults to scanning the bundled ``src_m`` tree.
"""
...
_SENSITIVE_PATTERNS: List[Tuple[str, str]] = [
    ...
    ("aws_secret_key", re.compile(
        r"""(?i)aws[_-]?secret[_-]?access[_-]?key\s*[:=]\s*['"](?P<value>[A-Za-z0-9/+=]{40})['"]"""
    )),
...
_CMD_INJECTION_PATTERNS: List[Tuple[str, re.Pattern]] = [
    ...
    ("subprocess_string_cmd", re.compile(
        r"""(?x)
...
# Default scan root: src_m tree.
_DEFAULT_SCAN_ROOT = "src_m"
...
def _scan_string_literal(value: str) -> Optional[Tuple[str, float]]:
    ...
    def _scan_line(
            ...
            for match in pattern.finditer(line):
                ...
                issues.append(
                    AnalysisIssue(
                        ...
                        description=(
                            f"疑似敏感信息: {label} (长度={len(snippet)})"
                        ),
                        suggestion=(
                            "将密钥迁移到环境变量、密钥管理服务或 .env 文件（已加入 .gitignore）"
                        ),
                        location=location,
                        details={
                            "kind": "sensitive",
        ...
        for label, pattern in self._patterns["deserialization"]:
            if pattern.search(line):
                severity = Severity.CRITICAL if label in {"pickle_load", "pickle_loads", "cPickle_load"} else Severity.HIGH
                issues.append(
                    AnalysisIssue(
                        severity=severity,
                        category=AnalysisCategory.SECURITY,
                        ...
                        suggestion=(
                            "对 pickle 使用来源可信的字节流或改用 JSON；"
                            "yaml.load 必须显式传入 Loader=SafeLoader"
                        ),
                        location=location,
            ...
            if pattern.search(line):
                ...
                issues.append(
                    AnalysisIssue(
                        ...
                        suggestion=(
                            "避免 shell=True；改用 shell=False + 列表参数并校验输入，"
                            "或使用 shlex.quote 包裹用户输入"
                        ),
                        location=location,
        ...
        if not in_comment:
            for match in _STRING_LITERAL.finditer(line):
                literal = match.group(0)[1:-1] if match.group(0)[0] != '"' or len(match.group(0)) < 4 else match.group(0)
                # Strip quotes for the entropy check.
                text_quote = match.group(0)
                if text_quote.startswith(('"""', "'''")):
                    inner = text_quote[3:-3]
                else:
                    inner = text_quote[1:-1]
                ...
                if result is not None:
                    ...
                    issues.append(
                        AnalysisIssue(
                            ...
                            category=AnalysisCategory.SECURITY,
                            description=(
                                f"字符串字面量具有高 Shannon 熵 ({entropy:.2f} bits)，疑似密钥/Token"
                            ),
                            suggestion=(
                                "若该字符串为凭证，请迁移到环境变量/密钥管理服务，"
                                "并在代码中仅引用其名称"
                            ),
                            location=location,
                            details={
