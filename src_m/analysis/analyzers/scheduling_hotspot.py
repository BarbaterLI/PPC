...
def _safe_get_metrics_collector() -> Optional[Any]:
...
def _coerce_metrics(raw: Any) -> Optional[Dict[str, Dict[str, Any]]]:
...
class SchedulingHotspotAnalyzer(BaseAnalyzer):
    ...
    async def analyze(self, context: Optional[Dict[str, Any]] = None) -> List[AnalysisIssue]:
        ...
        non_zero_throughputs = sorted(
            s.throughput for s in summaries if s.throughput > 0
        )
        if non_zero_throughputs:
            ...
            if len(non_zero_throughputs) % 2 == 1:
                ...
                cluster_median = (
                    non_zero_throughputs[mid - 1] + non_zero_throughputs[mid]
                ) / 2.0
        else:
            cluster_median = 0.0

        for s in summaries:
            if s.throughput >= self._spike_min_throughput and cluster_median > 0 and s.throughput >= cluster_median * self._spike_ratio:
                s.is_spike = True
        ...
        for s in summaries:
            ...
            issues.append(
                AnalysisIssue(
                    ...
                    suggestion=(
                        "考虑扩容该节点或调整调度权重，将部分任务分发到其他节点以平衡负载"
                    ),
        ...
        issues.insert(
            ...
            AnalysisIssue(
                ...
                details={
                    ...
                    "summaries": [s.to_dict() for s in sorted(
                        summaries, key=lambda x: x.task_count, reverse=True
                    )],
                },
            ),
        )
